<template>
    <div v-if="!can('list level-subject')">
        <forbidden/>
    </div>
    <div v-else>
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Level Subjects (Template)</h1>
                    </div>
                    <div class="col-sm-6">
                            ...
                            <table class="table table-head-fixed text-nowrap">
                                ...
                                <tbody>
                                    <tr v-for="(data, index) in option_users.data" :key="index">
                                        <td>{{data.level}}</td>
                                        <td>{{data.subject.code}} - {{data.subject.name}}</td>
                                        <td v-html="statusFormat(data.status)"></td>
                                        <td class="text-right">
                                            <button type="button" class="btn btn-primary btn-sm" @click="openEditModal(data)" v-if="can('edit section')"><i class="fas fa-edit"></i> Edit</button>
                                            <button type="button" class="btn btn-danger btn-sm" @click="remove(data.id)" v-if="can('delete section')"><i class="fas fa-trash-alt"></i> Remove</button>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
    ...
    <script>
    import addModal from "./AddLevelSubject.vue";
    import EditModal from "./EditLevelSubject.vue";
    export default {
        components: {
            addModal,
            EditModal,
        ...
        methods: {
            statusFormat(status){
                if(status == 'active'){
                    return '<small class="badge badge-success">Active</small>'
                }
                else{
                    return '<small class="badge badge-danger">Inactive</small>'
                }
            },
            ...
            getData(page){
                if (typeof page === 'undefined' || page.type == 'keyup'|| page.type == 'change'|| page.type == 'click') {
                    page = '/api/level-subject/list/?page=1';
                }
            ...
            remove(id){
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete('/api/level-subject/delete/'+id)
                        .then(response => {
                            Swal.fire(
                                'Deleted!',
