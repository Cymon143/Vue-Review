<template>
<div v-if="!can('list user') && !is('Super-Admin')">
    <forbidden/>
</div>
<div v-else>
                    ...
                    <div class="card-header p-3">
                        ...
                        <div class="card-tools float-left">
                            <div class="input-group input-group-sm" >
                                <select v-model="length" @change="getData" class="form-control form-control-sm" style="width: auto;">
                                    <option value="10">10</option>
                                    <option value="25">25</option>
                                    <option value="30">30</option>
                                </select>
                                <select v-model="status" @change="getData" class="form-control form-control-sm" style="width: auto;">
                                    <option value="">All Sub. status</option>
                                    <option value="1">Done</option>
                                    <option value="00">Pending</option>
                                </select>
                                <button class="btn btn-success btn-sm ml-auto" @click="openAddModal" v-if="can('create user')"><i class="fas fa-user-plus"></i> Add</button>
                            </div>
                        </div>
                        ...
                        <table class="table table-head-fixed text-nowrap">
                            ...
                            <tbody>
                                <tr v-for="(data, index) in option_users.data" :key="index">
                                    ...
                                    <td v-html="substituteStatus(data.status_substitute)"></td>
                                    <td class="text-right">
                                        <button type="button" class="btn btn-primary btn-sm" @click="openEditModal(data)" v-if="can('edit user')"><i class="fas fa-edit"></i> Edit</button>
                                        <button type="button" class="btn btn-danger btn-sm" @click="remove(data.id)" v-if="can('delete user')"><i class="fas fa-trash-alt"></i> Remove</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
...
export default {
    ...
    methods: {
        substituteStatus(status){
            if(status == 1){
                return '<small class="badge badge-success">Done</small>';
            }else{
                return '<small class="badge badge-danger">Not Set</small>';
            }
        },
        openAddModal() {
            $('#add-user').modal('show');
